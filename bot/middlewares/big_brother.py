            ...
            if message.voice or message.video_note:
                ...
                if media_repo and bot:
                    try:
                        ...
                        local_path = None
                        try:
                            save_dir = MEDIA_DIR / media_type
                            save_dir.mkdir(parents=True, exist_ok=True)
                            filename = f"{message.chat.id}_{message.message_id}.{ext}"
                            dest = save_dir / filename
                            await bot.download(media.file_id, destination=dest)
                            local_path = str(dest)
                            logger.info(f"[DOWNLOAD] Скачан {media_type}: {local_path}")
                        except Exception as e:
